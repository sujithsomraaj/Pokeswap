...
contract BOWL is PokeMath,IBOWL {
    
    string public constant name = "Bowl";
    ...
    function approve(address _spender, uint256 _amount) public override returns (bool) {
     ...
     function setrebase() public returns(bool){
         ...
         uint256 char = oracle.fetchChar();
         if(bowl>char){
             uint256 a = PokeMath.safeSub(bowl,char);
             uint256 b = PokeMath.safeMul(a,10 ** 19);
             uint256 c = PokeMath.safeDiv(b,char);
             rebase = c;
             pos = true;
             return true;
         }
         else{
             uint256 a = PokeMath.safeSub(char,bowl);
             uint256 b = PokeMath.safeMul(a,10 ** 19);
             uint256 c = PokeMath.safeDiv(b,bowl);
             rebase = c;
             pos = false;
             return false;
         }
